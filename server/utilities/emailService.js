...
const sendEmailVerificationOTP = async (user) => {
	sendEmail({
		from: " studyMeter Notification <verify@studyMeter.pk>",
		to: user.email,
		subject: "studyMeter Email Verification",
		templateObj: {
			name: user.name,
			otp: user.otp,
			email: user.email,
			emailText: `<p>Please verify that your email address is ${user.email} and that you entered it when signing up for studyMeter.</p>
